<?php
	...
	if($mouvement->getType() == 'livraison') {
	    ...
	    //ancienne valeur de la quantité en Entrepôt du gadget
	    $ancienneLivraison = Livraison::get($mouvement->getId());
	    $g = Gadget::get($ancienneLivraison->getIdGadget());
	    $qteEntrepôtGadget = $g->getQuantite() + $ancienneLivraison->getQuantite();
	    $valide = $l->getQuantite() <= $qteEntrepôtGadget;	//valide ssi la nouvelle qte à livrer est <= qteEntrepôtRéel du gadget
	    
		if(!$valide) {
			$_SESSION['status'] = Application::$ERREUR_AJOUT;
	    } else {
			$_SESSION['status'] = Application::$SUCCES_AJOUT;
			//mise a jour de la quantité en Entrepôt
			$g->setQuantite($qteEntrepôtGadget);	//réinitialisation de la qte du gadget
	    ...
	    $qteEntrepôtGadget = $g->getQuantite() - $ancienneReception->getQuantite();
		...
		if(!$valide) {
			...
			$_SESSION['status'] = Application::$SUCCES_AJOUT;
			//mise a jour de la quantité en Entrepôt
			$g->setQuantite($qteEntrepôtGadget);
			Gadget::modifier($g);	//la quantité du gadget est comme si on n'a pas fait de livraison
